...
/**
 * My Votes Page
 * Displays proposals voted by the current user
 */
...
export default function MyVotesPage() {
  ...
  const { getUserVotedProposals, getProposal } = useSecretBallot(provider, signer, chainId);
  ...
  const loadMyVotes = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      console.log(`🔍 Loading proposals voted by ${account}...`);
      
      const proposalIds = await getUserVotedProposals(account);
      console.log(`📊 Found ${proposalIds.length} voted proposals`);
  ...
  }, [account, getUserVotedProposals, getProposal]);

  useEffect(() => {
    if (isConnected && account) {
      loadMyVotes();
    }
  }, [isConnected, account, loadMyVotes]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex items-center justify-center">
        <div className="glass rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            Please connect your wallet to view your votes
          </p>
          <button
            onClick={() => router.push("/")}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-bold mb-4">{t("history.noVotes")}</h2>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {proposals.map((proposal) => (
              <div
                key={proposal.id}
                className="glass rounded-xl p-6 hover:shadow-xl transition-shadow cursor-pointer relative"
                onClick={() => router.push(`/proposal/${proposal.id}`)}
              >
                {/* Voted Badge */}
                <div className="absolute top-4 right-4">
                  <div className="px-3 py-1 bg-green-500 text-white text-xs font-medium rounded-full flex items-center gap-1">
                    ✓ {t("vote.voted")}
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600 dark:text-gray-400">{t("proposal.creator")}:</span>
